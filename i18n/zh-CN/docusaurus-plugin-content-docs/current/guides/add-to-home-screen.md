...
:::note
该功能需要 LiveContainer 3.6.62+。

请确保安装 LiveContainer 时至少包含 `LaunchAppExtension` 扩展（安装时选择 `Use Main Profile` 或 `Keep All Extensions`）。
:::

这是推荐的主屏幕添加方式。它可以直接启动应用，不需要先进入 LiveContainer 主界面，并且会自动寻找空闲的 LiveContainer 来打开应用。相比下面其他方法，每次启动大约可节省 1.5-3 秒。
...
6. 在 `Search Actions` 中搜索 `Open URLs`。请确认该动作打开的是上一步 `Launch App` 的输出结果。若不是，点击灰色 `URL`，然后在键盘上方选择 `Launch App`。（由于 iOS 限制后台进程打开 URL，无法将 Open URL 动作直接内嵌到 Launch App 动作中。）
7. 点击第一条动作中的灰色 `Launch URL` 文本，粘贴你在步骤 2 复制的启动 URL。
8. 点击 `x` 关闭键盘。
...
10. 回到快捷指令编辑页面。如果已关闭，点击该快捷指令右上角 `...` 重新打开。
11. 点击分享按钮，下滑并选择 `Add to Home Screen`。
12. 点击图标下方文本，修改显示名称。
13. 点击 `Image` -> `Choose File`，选择步骤 1 保存的图标文件。
14. 点击 `Add`，将图标添加到主屏幕。
15. 若要继续添加其他应用，可长按该快捷指令并点击 `Duplicate`，然后把 Launch URL 改成目标应用的新 URL。
<p>
...
这个方法与「Launch App 快捷指令」基本一致，区别只在「创建并配置快捷指令」这一步。

### 创建并配置快捷指令

3. 打开 iOS 的快捷指令（Shortcuts）App。如果找不到，请先从 App Store 下载。
4. 点击右上角 `+` 创建新快捷指令。
6. 在 `Search Actions` 中搜索 `Open URLs`。
